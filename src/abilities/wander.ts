import { ABILITY_TYPE } from "../constants";
...
newAbilityTrigger(ABILITY_TYPE.WANDER, EVENT.ABILITY_OBTAIN, (ability) => {
...
const t = newGlobalTrigger(EVENT.REPEAT_TIMEOUT, () => {
  for (let i = wanderers.length - 1 - offset; i >= 0; i -= 7) {
    ...
    if (
      unit.exists() && unit.isAlive() && unit.hasAbility(ABILITY_TYPE.WANDER)
    ) {
